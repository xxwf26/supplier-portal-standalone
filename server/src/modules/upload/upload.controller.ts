...
// 不含 .svg —— SVG 可内嵌脚本，经 /uploads 静态直出会造成存储型 XSS。
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        // 同时校验 mimetype 与扩展名；svg 已排除（存储型 XSS 风险）
        if (file.mimetype === 'image/svg+xml' || !file.mimetype.startsWith('image/') || !ALLOWED_EXT.has(ext)) {
          cb(new BadRequestException('只支持图片文件（jpg/png/gif/webp/bmp）'), false);
        } else {
          cb(null, true);
        }
      },
